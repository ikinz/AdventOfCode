...
def out(args):
	global output, done, alternating
	output += getData(args[0])

	if len(output) > 2000:
		done = True
	alternating = isAlternating(output)
...
with open("output_part1.txt", "w") as o:
	o.write(str(data["res"]))

print("Part1: " + str(data["res"]))
